...
tests_setup: tests/global_setup.sh
	@if [ -x "$<" ]; then env -i polygon_dir=${polygon_dir} PATH=$$PATH bash "$<"; fi

tests_teardown: tests/global_teardown.sh
	@if [ -x "$<" ]; then env -i polygon_dir=${polygon_dir} PATH=$$PATH /bin/bash "$<"; fi

test_%: tests/test_%
	@env -i TERM=$$TERM polygon_dir=${polygon_dir} PATH=$$PATH /bin/bash "$<"
